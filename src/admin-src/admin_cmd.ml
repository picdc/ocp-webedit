...
let make_db_speclist () =
  ...
  let create () =
    assert ((!current + 2) < l);
    if (!current + 3) < l then
      let name = argv_array.(!current + 3) in
      begin
  ...
  let chpsw () =
    assert ((!current + 3) < l);
    user_change_psw argv_array.(!current + 1) argv_array.(!current + 2)
      argv_array.(!current + 3);
    current := !current + 3
  in Arg.align
    ["-init", Unit db_initial,
     " Construct the data base of user information";
     "-create", Unit create,
     " -create EMAIL PASSWORD [NAME] creates a new user in the data base";
     "-delete", Unit delete,
     " -delete EMAIL deltes the user whose username is EMAIL";
     "-chpsw", Unit chpsw,
     " -chpsw EMAIL OLD_PASSWORD NEW_PASSWORD changes the password for user";
     "-list", Unit user_list,
     " -list print out information of all users (username [name] director)"]
...
let print_my_help () =
  ...
  print_endline
    "*****************************************************************";
  print_endline
    "*   This tool helps you with USER DATA MANAGEMENT, please run   *";
  print_endline
    "*                  admincmd database -help                     *";
  print_endline
    "* and also helps you with OCAML PACKAGES MANAGEMENT, please run *";
  print_endline
    "*                    admincmd opam -help                       *";
  print_endline
    "*****************************************************************";

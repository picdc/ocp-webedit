...
let read_prims filename =
  let ic = open_in filename in
  let len = in_channel_length ic in
  let buf = Buffer.create len in
  Buffer.add_channel buf ic len;
  close_in ic;
  let rec iter s pos0 pos1 =
    if pos1 = len then
      if pos0 < pos1 then
        set_prim_table (String.sub s pos0 (pos1 - pos0))
      else
        ()
    else
      let c = s.[pos1] in
      if c = '\n' then begin
        if pos0 < pos1 then
          set_prim_table (String.sub s pos0 (pos1 - pos0));
        iter s (pos1+1) (pos1+1)
      end
      else iter s pos0 (pos1+1)

  in
  iter (Buffer.contents buf) 0 0
...
(* Tags that could have been useful to js__of_ocaml
   to correctly recognize float at runtime
   Keeped in case it could be useful again
*)
type 'a tagged_value = { v : 'a }
...
let rec transl_const = function
  ...
  | Const_base(Const_float f) -> (* tag_value (float_of_string f)
    Obj.double_tag *)
    Obj.repr (float_of_string f)
